...
<?php foreach($form as $id => $f): ?>
  <?php if($id == "reply_author" && $f->getValue()!= ""): ?>
    <?php use_stylesheet("/myGalleryPlugin/css/replyTo.min.css", "last") ?>
  <?php endif ?>
  <?php if(!$f->isHidden()): ?>
    <?php if(!$f->hasError()): ?>
    ...
    <tr id="tr_reply_author_delete_<?php echo $form->getName() ?>" class="tr_reply_author_delete">
      <td colspan="2"><?php echo link_to_function(__("Delete the reply", array(), 'myGallery'), "deleteReply('".$form->getName()."')", array('class' => 'delete_reply'))."\n" ?></td>
    </tr>
    <?php endif ?>
  <?php endif ?>
<?php endforeach ?>
...
<?php echo $form->renderHiddenFields() ?>
        <input type="submit" value="<?php echo __('send', array(), 'myGallery') ?>" class="submit" />
      </td>
    </tr>
  </table>