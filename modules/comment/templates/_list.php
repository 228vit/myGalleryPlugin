...
<?php use_stylesheet("/myGalleryPlugin/css/comment.min.css") ?>
<?php use_stylesheet("/myGalleryPlugin/css/pagination.min.css") ?>
<?php use_javascript("/myGalleryPlugin/js/reply.min.js") ?>
<?php if(commentTools::isGravatarAvailable()): ?>
<?php use_helper('Gravatar') ?>
<?php endif ?>
  <div>
    <h1><?php echo __('Comments list', array(), 'myGallery') ?> (<?php echo $pager->getNbResults() ?>)</h1>
  </div>
...
<?php else: ?>
  <div>
    <h1><?php echo __('Be the first to post', array(), 'myGallery') ?></h1>
  </div>
<?php endif ?>